...
def test_invoke_does_not_raise_when_the_only_step_passes(capsys):
    @step("EXISTS")
    def exists(case):
        return Status.PASS
...
def test_invoke_does_not_raise_on_wait_by_default(capsys):
    @step("EXISTS")
    def exists(case):
        return Status.WAIT
...
def test_invoke_raises_on_fail(capsys):
    @step("EXISTS")
    def exists(case):
        return Status.FAIL
...
def test_invoke_raises_on_wait_when_fail_on_wait_is_set(capsys):
    @step("EXISTS")
    def exists(case):
        return Status.WAIT
...
def test_invoke_defaults_to_a_single_implicit_input_when_none_given(capsys):
    seen = []
...
def test_invoke_stops_at_the_frontier_step_for_that_input(capsys):
    calls = []
...
def test_invoke_continues_past_a_passing_step_to_the_next_one(capsys):
    calls = []
...
def test_invoke_evaluates_each_input_independently(capsys):
    @step("READY")
    def ready(case):
...
def test_invoke_raises_if_any_input_fails_even_if_others_pass(capsys):
    @step("READY")
    def ready(case):
...
def test_invoke_rejects_duplicate_step_names(capsys):
    @step("SAME_NAME")
    def first(case):
        return Status.PASS
...
def test_invoke_rejects_duplicate_input_names(capsys):
    @step("EXISTS")
    def exists(case):
        return Status.PASS
...
def test_invoke_rejects_a_step_that_was_not_decorated(capsys):
